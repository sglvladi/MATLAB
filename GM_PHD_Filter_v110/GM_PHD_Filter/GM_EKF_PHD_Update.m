...
for zi = 1:size(Z,2)
    ...
    for j = 1:numTargets_Jk_k_minus_1
        ...
        thisV = (inv_h(thisZ(1),thisZ(2),x_sensor(1), x_sensor(2), x_sensor(3)) - prevX(1:2)) / dt;%velocity = dx / dt. Since Z and x(1:2) are 2d, V = [Vx Vy]       
        thisZ = [thisZ; thisV];%So we pretend to observe velocity as well as position
        thisAugmentedZ(:, j + (zi - 1) * numTargets_Jk_k_minus_1) = thisZ;
...
%         new_weight = old_weight / (clutter_intensity(measZ) + weight_tally);%Normalise
        measZ_cartesian = inv_h(Z(1,zi), Z(2,zi), x_sensor(1), x_sensor(2), x_sensor(3));
        ...
        if(VERBOSE == 1)
            s = sprintf('\tNew target %d: Matching old target %d at %3.4f %3.4f to measurement %d at %3.4f %3.4f. Prob detection %3.4f. Old Weight %3.8f New Weight %3.8f Normalised New Weight: %3.8f', L * numTargets_Jk_k_minus_1 + j, j, mk_k_minus_1(1, j), mk_k_minus_1(2, j), zi, measZ_cartesian(1), measZ_cartesian(2), mvnpdf(Z(1:2,zi), eta(1:2,j), S(1:2,S_range(1:2))), wk_k_minus_1(j), old_weight, new_weight);
            disp(s);
        end
    end
