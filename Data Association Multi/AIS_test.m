...
[DataList, x1, y1] = gen_obs_AIS(AIS);
...
% 
    ...
    pf.gen_x0 = @(Np) [1000*rand(Np,1)-500,1000*rand(Np,1)-500, mvnrnd(zeros(Np,1), 2*sigma_v^2), 2*pi*rand(Np,1)];
    ...
    for i=1:TrackNum
       pf.gen_x0 = @(Np) mvnrnd(repmat([x1(1,1),y1(1,i),0,0],Np,1),diag([q^2, q^2, (Vmax^2/3), 180])); 
       %pf.gen_x0 = @(x) mvnrnd([x1(1,1),y1(1,i),0,0],diag([sigma_v^2, sigma_v^2, 2*sigma_v^2, 2*sigma_v^2]));
       pf.xhk = [x1(1,1),y1(1,i),0,0]';
       pf.ExistProb = 0.8;
...
%         [TrackList, betta] = JPDAF_UKF_Update(TrackList, tempDataList, ValidationMatrix', bettaNTFA);
        ...
        ValidationMatrix = zeros(ConfTrackNum, size(tempDataList,2)); 
        tot_gate_area = 0;
        for t = 1:ConfTrackNum
            TrackListPF{t}.TrackObj.pf.k = Dt;
...
%         del_tracks = 0;
%         for t = 1:ConfTrackNum
%             if(TrackListPF{t}.TrackObj.pf.ExistProb<0.1)
%                 TrackListPF{t} = [];
...
%         end
%         if(del_flag)
%             TrackListPF = TrackListPF(~cellfun('isempty',TrackListPF));
%             ConfTrackNum = ConfTrackNum - del_tracks;
%         end
%         
...
%             TrackListSearch{1}.TrackObj.pf.k = Dt;
...
%                     end
                    ...
                    %axis([5000000 5100000 -230000 -220000])
