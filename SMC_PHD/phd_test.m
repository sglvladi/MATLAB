...
[DataList,x1,y1] = gen_obs_cluttered_multi3(TrackNum, x_true, y_true, r, lambdaV, 1); 
...
config.gen_x0 = @(Np) [(V_bounds(2)-V_bounds(1))*rand(Np,1),(V_bounds(4)-V_bounds(3))*rand(Np,1), mvnrnd(zeros(Np,1), q^2), 2*pi*rand(Np,1)]; % Uniform position and heading, Gaussian speed
config.particles = config.gen_x0(config.Np)'; % Generate inital particles as per gen_x0
config.w = repmat(1/config.Np, config.Np, 1)'; % Uniform weights
...
config.birth_strategy = 'expansion';

% Instantiate PHD filter
myphd = SMC_PHD(config);
