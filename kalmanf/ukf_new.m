function [x,P]=ukf_new(fstate,x,P,hmeas,z,Q,R)
...
%Wm
%Wc
%[Wm,Wc] = sigma_weights(n, alpha, ki, beta);
%Wm_test
%Wc_test
Wm
Wc
...
%P
%X = sigma(x, P, alpha, ki)
%P
[x1,X1,P1,X2]=ut(fstate,X,Wm,Wc,n,Q);          %unscented transformation of process
Z=sigma(x1, P1, alpha, ki);                         %sigma points around x1
% X2=X1-x1(:,ones(1,size(X1,2)));             %deviation of X1
[z1,Z1,P2,Z2]=ut(hmeas,Z,Wm,Wc,m,R);       %unscented transformation of measurments
...
function X=sigma(x, P, alpha, kapa)
n = numel(x);
...
for i=n+1:2*n
    X(:,i+1) = x - real(MSR(:,i-n));
end

function [Wm, Wc]=sigma_weights(n, alpha, kapa, beta)
lambda = alpha^2*(n+kapa)-n;
Wm = zeros(1, 2*n+1);
