...
%     s.sys=@(t)(@(x)[x(1)+ t*x(3); x(2)+t*x(4); x(3); x(4)]);
    ...
    gen_sys_noise_ccv = @(u) mvnrnd(zeros(1, nu), cov_u);
    gen_sys_noise_ccv2 = @(u) mvnrnd(zeros(1, nu), 2*cov_u);
    ...
    pf_ccv_opt = ParticleFilterMin(pf);
    ...
    pf.kf.s.sys = pf.kf.s.sys(Dt);
    pf.sys_noise = gen_sys_noise_cch;
    pf_cch_opt = ParticleFilterMin(pf);
...
plot(1:N, permute(err(1,6,:),[2 3 1]), 'g-', 'LineWidth', 1)
