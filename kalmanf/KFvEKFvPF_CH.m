...
N=100;%size(x,2);                                     % total dynamic steps
...
sys = @(k, xkm1, uk) [xkm1(1)+Dt*xkm1(3)+ uk(1); xkm1(2)+Dt*xkm1(4) + uk(2); xkm1(3)+ uk(3); xkm1(4) + uk(4)]; % (returns column vector)
...
%% Compute & Print RMSE
eV_kf = (xV_kf(1:2,:) - x(:,1:100)).*(xV_kf(1:2,:) - x(:,1:100));
eV_ekf = (xV_ekf(1:2,:) - x(:,1:100)).*(xV_ekf(1:2,:) - x(:,1:100));
eV_ukf = (xV_ukf(1:2,:) - x(:,1:100)).*(xV_ukf(1:2,:) - x(:,1:100));
...
%% Compute RMSE
pf_err = (xh(1:2,:) - x(:,1:100)).*(xh(1:2,:) - x(:,1:100));
err = (zV_ukf(:,:) - x(:,1:100)).*(zV_ukf(:,:) - x(:,1:100));
