...
sys = @(k, xkm1, uk) [xkm1(1)+1*xkm1(3)*cos(xkm1(4)); xkm1(2)+1*xkm1(3)*sin(xkm1(4)); xkm1(3)+ normrnd(0,0.1); xkm1(4) + normrnd(0, 0.2)]; % (returns column vector)
sys2 = @(k, xkm1, uk) [xkm1(1)+1*xkm1(3); xkm1(2)+1*xkm1(4); xkm1(3)+ 0.1*normrnd(0,1); xkm1(4) + 0.1*normrnd(0,1)]; % (returns column vector)
...
y = [obs_x'; obs_y'];
%% Separate memory
xh = zeros(2,nx, T); xh(1,:,1) = xh0; xh(2,:,1) = xh0; 
yh = zeros(2,ny, T); yh(1,:,1) = obs(1, xh0, zeros(1, nv)); yh(2,:,1) = obs(1, xh0, zeros(1, nv));
...
for k = 2:T
   ...
   xh(1,:,k) = my_pf.pf.xhk(:,k);
   % filtered observation
   yh(1,:,k) = obs(k, xh(1,:,k), zeros(1, nv));
   ...
   xh(2,:,k) = my_pf2.pf.xhk(:,k);
   % filtered observation
   yh(2,:,k) = obs(k, xh(2,:,k), zeros(1, nv));
...
%    end
   ...
   h3 = plot(permute(xh(1,1,1:k),[2 3 1]),permute(xh(1,2,1:k),[2 3 1]),'m','LineWidth',1);
   h4 = plot(y(1,1:k), y(2,1:k),'g.','LineWidth',1);
   h5 = plot(permute(xh(1,1,1:k),[2 3 1]),permute(xh(1,2,1:k),[2 3 1]),'m.','LineWidth',3);
   axis([0 10 0 10])
   ...
   %hold on;
   %h2 = plot(x(1,1:k),x(2,1:k),'b','LineWidth',1);
   h6 = plot(permute(xh(2,1,1:k),[2 3 1]),permute(xh(2,2,1:k),[2 3 1]),'r','LineWidth',1);
   %h4 = plot(y(1,1:k), y(2,1:k),'g.','LineWidth',1);
   h7 = plot(permute(xh(2,1,1:k),[2 3 1]),permute(xh(2,2,1:k),[2 3 1]),'r.','LineWidth',3);
   axis([0 10 0 10])
   legend([h2 h3 h4],'state','mean of estimated state','measurements');
   title('State vs estimated state by the particle filter vs particle paths','FontSize',14);
