...
for test_iter = 1:size(r_list,2)
    ...
    for sim_iter = 1:Sim_No
        ...
        TrackNum = 1 ;
        [DataList,x1,y1] = gen_obs_cluttered_multi2(TrackNum, x_true, y_true, r_list(1,test_iter), 16, 30);
...
%         s.Q = diag([0,0,0.01^2,0.3^2]);
        ...
        for i=1:TrackNum,
            ...
            pf.gen_x0 = @(x) mvnrnd([x1(2,1); y1(2,1); sqrt((x1(2,1)-x1(1,1))^2+(y1(2,1)-y1(1,1))^2); atan((y1(2,1)-y1(1,1))/(x1(2,1)-x1(1,1)))],diag([sigma_v^2, sigma_v^2, 2*sigma_v^2, 2*sigma_v^2]));
        ...
        for i=2:N
            ...
            h2 = plot(DataList{i}(1,:),DataList{i}(2,:),'k*','MarkerSize', 20);
            h2 = plot(DataList{i}(1,1),DataList{i}(2,1),'r*','MarkerSize', 20);
