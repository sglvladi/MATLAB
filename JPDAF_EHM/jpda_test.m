...
%                           (m: time, n: targets)* 
%
%          *Example datasets can be found under "Example datasets" folder  
%   
%   Dependencies: gen_obs_cluttered_multi.m, UKalmanFilter.m,
%                 Observation Association.m, JPDAF_EHM_Update.m 
%   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
for sim = 1:SimNum
    ...
    % Option 1) Two-point difference initiation
    %for i = 1:TrackNum
    ...
    % Option 2) Single-point initiation
    Vmax = 0.4;     % Assume max velocity = 0.4 m/s
    for i = 1:TrackNum
        s.x_init(:,i)=[x1(1,i); y1(1,i); 0; 0]; %initial state
    end
    s.P_init = diag([q^2, q^2, q^2, q^2]);
    ...
    for i=1:N
        ...
        [TrackList, ValidationMatrix, bettaNTFA] = Observation_Association(TrackList, tempDataList, ukf);
        
        % JPDAF_UKF_Update provides JPDAF updates for all targets, using a 
        %   UKF.
        TrackList = JPDAF_EHM_Update(TrackList, tempDataList, ValidationMatrix', bettaNTFA);
        %TrackList = Track_InitConfDel(TrackList,tempDataList,ValidationMatrix');
        ...
        % Plotting routine (Set ShowPlots=0 to disable)
        if (ShowPlots)
            % Plot data
...
h2 = plot(DataList{end}(1,:),DataList{end}(2,:),'k*','MarkerSize', 10);
for j=1:TrackNum,
    colour = 'r';
    if(j==2)
        colour = 'c';
