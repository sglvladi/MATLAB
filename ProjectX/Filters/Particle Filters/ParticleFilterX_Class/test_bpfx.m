...
config_pf.k = 1;
config.Np = 10000;
config_pf.gen_x0 = @(Np) mvnrnd(repmat([x_true(2,1); y_true(2,1); x_true(2,1)-x_true(1,1); y_true(2,1)-y_true(2,1)]', Np,1), diag([config_meas.r^2, config_meas.r^2, 0, 0])+CHmodel.config.Q(1));
pf = ParticleFilterX(config_pf, CVmodel, obs_model);
...
for k = 1:N
  ...
  xV(:,k) = pf.config.x;                    % save estimate
  filtered_estimates{k} = pf.config;
