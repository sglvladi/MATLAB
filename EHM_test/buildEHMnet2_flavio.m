...
function NetObj = buildEHMnet2(ValidationMatrix, Li)
    ...
    % It turns out that Flávio gave the wrong advice, take this one instead
    % (by Flávio 24/05/2017) - beer when I come to Liverpool for a visit?
    betta_modified = betta(:,2:end)';
    betta_transpose = [1-sum(betta_modified,2),betta_modified];
    betta_transpose = [ones(1,size(betta,2)-1)-sum(betta(:,2:end),1); betta(:,2:end)]'
